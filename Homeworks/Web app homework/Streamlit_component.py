    ...
    if testing_mode == 1:
        if a > b:
            ...
            while(a > b):
                a = int(input("a: "))
                b = int(input("b: "))
    iterations_cnt = 100 if testing_mode == 1 else st.number_input("Max iterations: ", value=100, step=10)
    formula = "x**2 - 5*sin(x) + x - 1" if testing_mode == 1 else st.text_input("Enter function (Example format, x**2 - 5*sin(x) + x - 1): ", value="x**2 - 5*sin(x) + x - 1")
    tolerance = 1e-6 if testing_mode == 1 else st.number_input("Tolerance: ", value=1e-6, format="%.1e")
